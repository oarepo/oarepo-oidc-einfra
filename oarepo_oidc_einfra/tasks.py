...
def replace_resource_capability(
    ...
    *,
    ...
    if not resource:
        log.warning(
            "Resource for capability %s not found inside Perun, so can not replace it.",
            old_capability,
        )
        return
    ...
    for aai_user_chunk in chunks(dump.users(), 100):
        ...
        for user in local_users:
            ...
            new_community_roles = filter_community_roles(
                community_support, aai_user.community_roles
            )
...
def aai_group_op(
    ...
    for group in perun_api.get_resource_groups(resource_id=resource["id"]):
        try:
            ...
            log.exception(
                "Error while performing %s on group %s for user %s", op, group, user
            )
