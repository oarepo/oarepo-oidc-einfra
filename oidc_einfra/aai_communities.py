...
from oidc_einfra.utils import get_identity_einfra_id, CommitOp
...
class AAICommunities:
    ...
    @unit_of_work()
    def create_community(self, community_slug,
                         community_name,
        ...
        if not community:
            self.communities_service.create(self.identity, {
                "slug": community_slug,
                "access": {
        ...
        self.update_community_aai_groups(community.id, community_name, community_description, uow=uow)

    @unit_of_work()
    def update_community_aai_groups(self, community_id, community_name, community_description, uow=None):

        mappings = {
            x.role: x for x in CommunityAAIMapping.query.filter_by(community_id=community_id).all()
        }

        mappings["uuid"] = self._fix_group(
            community_id=community_id,
            role='community',
            mapping=mappings.get("uuid"), group_name=community_name, group_description=community_description, uow=uow)
        ...
        mappings["members"] = self._fix_group(
            community_id=community_id,
            role='members',
            mapping=mappings.get("members"), group_name="members", group_description="Community members",
            parent=community_aai_group, uow=uow)
        self._fix_group_admins(mappings["members"].aai_group_uuid, parent=community_aai_group)

        mappings["curators"] = self._fix_group(
            community_id=community_id,
            role='curators',
            mapping=mappings.get("curators"), group_name="curators", group_description="Community curators", parent=community_aai_group, uow=uow)
        self._fix_group_admins(mappings["curators"].aai_group_uuid, parent=community_aai_group)

        mappings["reviewers"] = self._fix_group(
            community_id=community_id,
            role='reviewers',
            mapping=mappings.get("reviewers"), group_name="reviewers", group_description="Community reviewers (mark records as reviewed)", parent=community_aai_group, uow=uow)
        self._fix_group_admins(mappings["reviewers"].aai_group_uuid, parent=community_aai_group)

        mappings["publishers"] = self._fix_group(
            community_id=community_id,
            role='publishers',
            mapping=mappings.get("publishers"),
            group_name="publishers", group_description="Community publishers (publishes the record)", parent=community_aai_group, uow=uow)
        self._fix_group_admins(mappings["publishers"].aai_group_uuid, parent=community_aai_group)
    ...
    def get_community_members(self, community_id, role):
        return {member.user_id for member in MemberModel.query.filter_by(community_id=community_id, role=role)}
    ...
    def get_aai_users(self, vo_uuid, group_uuid):
        ...
        return {
            ui.id_user for ui in UserIdentity.query.filter(
                UserIdentity.method == "e-infra",
                UserIdentity.id.in_(einfra_ids)
            )
        }
    ...
    def _fix_group(self, *, community_id, mapping: CommunityAAIMapping,
                   role, group_name, group_description, parent=None, uow=None):
        ...
        if group is None:
            ...
            group = None
            try:
                group = next(x for x in parent_group.subgroups.values() if x.metadata['shortName'] == group_name)
            except StopIteration:
                pass

            if not group:
                group = parent_group.subgroups.create(name=group_name, description=group_description)
    ...
    def send_invitation(self, email, language):
        ...
        resp = self.aai_api.send_request("POST", "registrarManager", "sendInvitation",
                                         voId=vo.id,
    ...
    def _fix_group_admins(self, group_id, parent=None):
        ...
        admins_to_create = [
            admin for admin in communities_group.admins.values() if admin.uuid not in grp.admins
        ]
