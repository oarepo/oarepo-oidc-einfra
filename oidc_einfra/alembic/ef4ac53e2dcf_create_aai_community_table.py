...
def upgrade():
    """Upgrade database."""
    op.create_table('communities_aai_mapping',
    ...
    sa.Column('aai_group_uuid', sqlalchemy_utils.types.uuid.UUIDType(), nullable=True),
    sa.Column('managed', sa.Boolean(), nullable=False),
    sa.Column('community_id', sqlalchemy_utils.types.uuid.UUIDType(), nullable=False),
    sa.ForeignKeyConstraint(['community_id'], ['communities_metadata.id'], name=op.f('fk_communities_aai_mapping_community_id_communities_metadata'), ondelete='CASCADE'),
